  ...
  if (error) {
    return (
      <div className="min-h-screen py-8 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-red-900/50 border border-red-800 rounded-lg p-6 mb-8">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400">Dashboard</h1>
              <p className="text-gray-400">Welcome back, {currentUser?.fullName || 'User'}!</p>
            </div>
            <div className="mt-4 sm:mt-0">
              <select
                value={timeRange}
                onChange={(e) => setTimeRange(e.target.value)}
                className="px-3 py-2 border border-gray-700 bg-gray-800 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
              ...
              <Card.Content>
                <div className="space-y-3">
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        const token = localStorage.getItem('token');                        const response = await fetch('/api/user/history/export/all', {
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        const token = localStorage.getItem('token');                        const response = await fetch('/api/user/history/export/scans', {
                          headers: { 'Authorization': `Bearer ${token}` }
                        });
                        const data = await response.json();
                        const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
